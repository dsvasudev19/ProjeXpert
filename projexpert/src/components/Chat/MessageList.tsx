...
const MessageList = ({ messages, loadMore, hasMore, isInitialLoad }: MessageListProps) => {
  ...
  if (loading) return (
    <div className="flex-1 flex flex-col items-center justify-center p-8 bg-[#fffcef] h-full">
      <div className="flex items-center space-x-2 mb-2">
        <div className="w-3 h-3 rounded-full bg-[#5c899D] animate-bounce" style={{ animationDelay: '0ms' }}></div>
        <div className="w-3 h-3 rounded-full bg-[#5c899D] animate-bounce" style={{ animationDelay: '150ms' }}></div>
        <div className="w-3 h-3 rounded-full bg-[#5c899D] animate-bounce" style={{ animationDelay: '300ms' }}></div>
      </div>
      <p className="text-[#5c899D] font-medium">Loading messages...</p>
    </div>
  );
  ...
  if (!messages || messages.length === 0) {
    return (
      <div className="flex-1 flex flex-col items-center justify-center p-8 h-full" style={{ backgroundColor: '#fffcef' }}>
        <div className="w-20 h-20 bg-white rounded-full flex items-center justify-center mb-5 shadow-sm border-2" style={{ borderColor: '#5c899D', backgroundColor: '#fffcef' }}>
          <svg className="w-10 h-10" style={{ color: '#5c899D' }} fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="1.5" d="M8 12h.01M12 12h.01M16 12h.01M21 12c0 4.418-4.03 8-9 8a9.863 9.863 0 01-4.255-.949L3 20l1.395-3.72C3.512 15.042 3 13.574 3 12c0-4.418 4.03-8 9-8s9 3.582 9 8z"></path>
          </svg>
        </div>
        <h3 className="text-xl font-semibold mb-2" style={{ color: '#5c899D' }}>No Messages Yet</h3>
        <p className="text-gray-600 text-center max-w-xs">Start a conversation to see your message history here</p>
      </div>
    );
  }
